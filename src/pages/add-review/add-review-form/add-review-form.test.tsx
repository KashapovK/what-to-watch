...
describe('Component: AddReviewForm', () => {
  ...
  it('should submit review', async () => {
    const { component, mockStore, mockHistory, mockAxiosAdapter } = withProviders(
      <Routes>
        <Route path={AppRoute.AddReview} element={<AddReviewForm />} />
        <Route path={AppRoute.Film} element={<AddReviewForm />} />
      </Routes>
    );
    mockAxiosAdapter.onPost(/\/comments/).reply(StatusCodes.OK);
    mockHistory.push(AppRoute.AddReview.replace(':id', mockFilm.id));
    render(component);
    ...
    expect(mockHistory.location.pathname).toBe(AppRoute.Film.replace(':id', mockFilm.id));
  });
